...
/**
 * Executes the entire metadata extraction pipeline for a given candidate image or video.
 */
export async function extractImageContext(el: HTMLElement, url: string): Promise<ImageContext> {
  ...
  if (el instanceof HTMLVideoElement) {
    context.width = el.videoWidth || undefined;
    context.height = el.videoHeight || undefined;
    context.dimensions = el.videoWidth ? `${el.videoWidth} × ${el.videoHeight} px` : 'unknown dimensions';
    if (el.duration) {
      context.duration = formatDuration(el.duration);
    }
...
export function createSidebarDOM(context: ImageContext, onClose: () => void): string {
  ...
  const isVideo = !!context.duration || !!context.poster || (context.sourceUrl && context.sourceUrl.match(/\.(mp4|webm|ogg|mov)(\?|$)/i));
    ...
    <div class="sidebar-content">
      ...
      ${context.duration ? `
        <div class="sidebar-section">
          ...
          <span class="section-value" style="font-weight: 500;">${context.duration}</span>
        </div>
      ` : ''}

      ${context.poster ? `
        <div class="sidebar-section">
          <span class="section-label">Poster URL</span>
          <span class="section-value" style="word-break: break-all; font-size: 11px; opacity: 0.8;" title="${context.poster}">${context.poster}</span>
        </div>
      ` : ''}
